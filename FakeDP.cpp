...
void FakeDP::Service(uint32_t timeout)
{
	ENetEvent evt;
...
HRESULT_INT FakeDP::InitializeConnection(LPVOID lpConnection, DWORD dwFlags)
{
	...
	if (InlineIsEqualGUID(addr->guidDataType, GUID_ENet))
		return DP_OK; // Skip internal GUID
...
bool FakeDP::GetAddressFromDPAddress(LPVOID lpConnection, ENetAddress* out)
{
...
HRESULT_INT FakeDP::EnumConnections(LPCGUID lpguidApplication, LPDPENUMCONNECTIONSCALLBACK lpEnumCallback, LPVOID lpContext, DWORD dwFlags)
{
	...
	DPADDRESS dummyAddr;
	dummyAddr.dwDataSize = 0;
	dummyAddr.guidDataType = GUID_ENetAddress;

	// TODO: Figure out why it's crashing
	//lpEnumCallback(&GUID_ENet, &dummyAddr, sizeof(dummyAddr), &dpName, 0, lpContext);
